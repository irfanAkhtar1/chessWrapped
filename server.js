...
// ─── FETCH PLAYER PROFILE (avatar + ratings) ──────────────────────────────────
...
app.get('/api/stats/:username', async (req, res) => {
  ...
  try {
    ...
    for (const g of games) {
      ...
      if (result === 'win') { wins++; bestStreak = Math.max(bestStreak, ++streak); }
      else streak = 0;
    ...
    // Roast
    const roast = total > 3000 ? 'Bro is farming Chess.com like a job 💀'
      : winRate < 45 ? 'You donate pieces professionally.'
        : winRate > 65 ? 'Okay Magnus, relax.'
          : 'Solid player. Nothing to roast here.';

    res.json({
      username, avatarUrl, totalGames: total, winRate, bestStreak,
      timeControlBreakdown: breakdown, ratings, personality, roast
    });
...
app.listen(PORT, () => console.log(`♟  Chess Wrapped → http://localhost:${PORT}`));
